...
#Add another column with Stable or Aperiodic
total_df$Classification =c(rep("Stable",201),rep("Aperiodic",201))



#K-Means
kmeans(total_df$A, 2,25)

#ConfusionMatrix
kmtotal_df <- kmeans(total_df$A, 2,25)
km_clusters <- kmtotal_df$cluster
correct <- rep(c("+","-"), each=201)
table(km_clusters,correct)

#Takens Embedding theorem
suppressMessages(library('nonlinearTseries'))
total_a = total_df$A
tak = buildTakens(total_a,embedding.dim = 2, time.lag = 3)
plot(tak)

#TDA Persistent Homology - Barcode and persistence diagram
